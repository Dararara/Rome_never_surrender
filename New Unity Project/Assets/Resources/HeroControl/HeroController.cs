...
public class HeroController : MonoBehaviour
{
    ...
    IEnumerator HitEnemy()
    {
        ...
        Collider[] colliderArr = Physics.OverlapSphere(gameObject.transform.position, 5, LayerMask.GetMask("Enemy"));
        for (int i = 0; i < colliderArr.Length; i++)
        {
            ...
            {
                Debug.Log("attack");
                colliderArr[i].gameObject.GetComponent<EnemyControl>().GetHit(5f);
